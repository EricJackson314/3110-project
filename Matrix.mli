(** Finite-dimensional matrices with basic algebraic functions.
...
module type Matrix = sig
  ...
  (** Representation of the matrix's elements. *)
  module E : Num
  (** Representation of the vectors that the this matrix module can 
      interact with. *)
  ...
  (** Raised when the dimensions of a matrix do not match those required by a 
      function. *)
  exception DimensionMismatchException

  (** Raised when an attempt is made to access an index which is out of 
      bounds. *)
  exception OutOfBoundsException

  (** Raised if a function requiring a non-singular matrix is passed a singular 
      matrix. *)
  ...
  (** [make r c e] is [x] if x has r rows, c columns, and the entry a b x =
      ...
      is called, so that if random number generation or mutable state is 
      involved in the function e, entry may not return the same value on each 
      call. *)
  val make : int -> int -> (int -> int -> elem) -> t

  (** [make_abs r c e] is [x] if x has r rows, c columns, and 
      entry a b x = e a b. Unlike [make r c e], this function evaluates e only
      once for each entry so the entry function is guaranteed to return the 
  ...
  (** [mult mat1 mat2] is the matrix product mat1 * mat2. 
      Requires: (num_cols mat1 = num_rows mat2).
      Raises: [DimensionMismatchException] *)
  val mult : t -> t -> t

  (** [add mat1 mat2] is the matrix addition mat1 + mat2. 
      Requires: mat1 and mat2 to have the same number of rows and columns
      Raises: [DimensionMismatchException] *)
  val add : t -> t -> t
  ...
  (** [transpose mat] is [x : Matrix.t] such that entry a b mat = entry b a x 
      for all a and b *)
  val transpose : t -> t
  ...
  (** [scale_row r scalar mat] is [x] if the entries of x are the same as mat
      ...
      multiplied by scalar. Raises OutOfBoundsException if r is out of 
      bounds. *)
...
(** Creates a matrix module containing Elements of type [Elem.t]. *)
